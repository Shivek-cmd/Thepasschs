    ...
    <section className="section-xl relative w-full overflow-hidden" style={{ background: 'var(--color-bg-elevated)' }}
      ...
      <div className="absolute inset-0" aria-hidden="true">
        <Image src="https://images.squarespace-cdn.com/content/v1/66c6970e298efb4cf210f0ff/09302024_The-Pass_Andrew-Cebulka-10510-2.jpg"
          ...
          fill className="object-cover opacity-30" sizes="100vw" />
      </div>
      <svg className="hidden md:block absolute right-16 top-1/2 -translate-y-1/2 pointer-events-none"
      ...
      <div className="absolute top-1/2 left-1/4 -translate-y-1/2 w-[400px] h-[400px] rounded-full pointer-events-none"
        style={{ background: 'rgba(201,168,76,0.06)', filter: 'blur(100px)' }} aria-hidden="true" />
      ...
      <div className="site-container relative z-10">
        <div style={{ maxWidth: '640px', margin: '0 auto', textAlign: 'center' }}>
          ...
          <AnimatedSection delay={0.15}>
            <h2 id="italianboy-heading" className="sr-only">The Italian Boy After Dark</h2>
            <p style={{ fontSize: '1.25rem', lineHeight: 1.6, color: 'rgba(240,235,227,0.85)', marginBottom: '1.25rem', fontFamily: 'var(--font-display)', fontStyle: 'italic' }}>
              Nothing like the daytime.
            </p>
            <p style={{ fontSize: '1rem', lineHeight: 1.8, color: 'rgba(240,235,227,0.62)', marginBottom: '3rem', maxWidth: '560px', margin: '0 auto 3rem' }}>
          ...
          <AnimatedSection delay={0.2}>
            <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'center', gap: '1rem', marginBottom: '2rem' }}>
              <MagneticButton>
                <a href={SITE.resy} target="_blank" rel="noopener noreferrer"
                  className="inline-flex items-center gap-2.5 font-semibold transition-all duration-200 hover:shadow-glow"
                  style={{ background: 'var(--color-accent)', color: '#1C1A17', padding: '0.875rem 2rem', borderRadius: '9999px', fontSize: '0.9375rem' }}>
                  Reserve on Resy <ArrowRight className="w-4 h-4" />
                </a>
              </MagneticButton>
              <Link href="/the-italian-boy"
                className="inline-flex items-center gap-2 font-semibold transition-all duration-200"
                style={{ border: '1px solid rgba(240,235,227,0.25)', color: 'rgba(240,235,227,0.85)', padding: '0.875rem 2rem', borderRadius: '9999px', fontSize: '0.9375rem' }}>
                Learn More
              </Link>
            </div>
            <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: '0.625rem' }}>
              {['$120 / Person', '12 Seats', '2 Hours', 'Prepaid via Resy'].map(tag => (
                <span key={tag} className="tag-pill" style={{ background: 'rgba(240,235,227,0.07)', color: 'rgba(240,235,227,0.60)', border: '1px solid rgba(240,235,227,0.10)' }}>
                  {tag}
                </span>
              ))}
            </div>
